class OrdersController < ApplicationController
  ...
  before_action :validate_session, only: [:delivery,:saveDelivery,:summary]
  before_action :validate_value_in_session, only: [:summary]
  #before_action :authenticate_client!, only: [:delivery,:saveDelivery,:summary,:payment]
  before_action :authenticate_client!, only: [:delivery,:saveDelivery,:summary]
...
# ==========================  ENREGISTREMENT DES COMMANDE =========    
      ...
      begin
        ...
        logger.info "\n\n ################################# \n ERROR new_order_relance_client ORDER \n"
  ...
  def payment
    # @order = current_client.orders.order('id ASC').last
    @order = Client.find(params[:client_id]).orders.order('id ASC').last
    ...
    data.each do |params|
      if params.include?("paymentMeanBrand=")
        value_p = params.split("=")[1]
      end
      if params.include?("transactionDateTime")
        value_t = params.split("=")[1]
      end
      if params.include?("amount")
        value_a = params.split("=")[1]
      end
    end
    @order.update(paymentMeanBrand:value_p,transactionDateTime:value_t,acompte_amount:value_a)
    ...
    if data.include?("responseCode=00")
      ...
      begin
        begin
          ...
          logger.info "\n\n ################################# \n ERROR CONFIRM ORDER \n\n"
          DeveloperMailer.notify_unsuccessfull_mail(exception, current_client.id).deliver_now
          logger.debug "[ERROR APPLICATION MAILER] Class: #{exception.class},\nMessage #{exception.full_message(highlight: true, order: :top)}"
        end
